...
class KkybankPipeline:

    # Database setup
    conn = sqlite3.connect('kybank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `kybank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM kybank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO kybank (date, title, link, content)"
